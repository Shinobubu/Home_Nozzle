...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/home_nozzle.py" "${KLIPPER_PATH}/klippy/extras/home_nozzle.py"
    echo "[OK]"
}
...
remove_service()
{
    SERVICE_FILE="${SYSTEMDDIR}/home_nozzle.service"
    if [ -f "$SERVICE_FILE" ]; then
        echo -n "Removing system service... "
        sudo service home_nozzle stop
        sudo systemctl disable home_nozzle.service
        sudo rm "$SERVICE_FILE"
        echo "[OK]"
    fi
    OLD_SERVICE_FILE="${SYSTEMDDIR}/klipper_home_nozzle.service"
    if [ -f "$OLD_SERVICE_FILE" ]; then
        echo -n "Removing old system service... "
        sudo service klipper_home_nozzle stop
        sudo systemctl disable klipper_home_nozzle.service
...
add_updater()
{
    ...
    update_section=$(grep -c '\[update_manager[a-z ]* home_nozzle\]' $MOONRAKER_CONFIG || true)
    if [ "$update_section" -eq 0 ]; then
        echo -e "\n[update_manager home_nozzle]" >> "$MOONRAKER_CONFIG"
        echo "type: git_repo" >> "$MOONRAKER_CONFIG"
        echo "path: ${SRCDIR}" >> "$MOONRAKER_CONFIG"
        echo "origin: https://github.com/Shinobubu/ZOffsetFinder.git" >> "$MOONRAKER_CONFIG"
...
uinstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/home_nozzle.py" ]; then
        echo -n "Uninstalling home_nozzle... "
        rm -f "${KLIPPER_PATH}/klippy/extras/home_nozzle.py"
        rm -f "${KLIPPER_PATH}/klippy/extras/home_nozzle.pyc"
        echo "[OK]"
        echo "You can now remove the \"[update_manager home_nozzle]\" section in your moonraker.conf and delete this directory."
        echo "You also need to remove the \"[home_nozzle]\" section in your Klipper configuration..."
    else
        echo -n "${KLIPPER_PATH}/klippy/extras/home_nozzle.py not found. Is it installed? "
        echo "[FAILED]"
    fi
}
